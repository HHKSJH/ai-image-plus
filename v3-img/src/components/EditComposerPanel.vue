...
function handleFileChange(event) {
  emit("upload-image", event.target.files?.[0] || null);
}
</script>
  ...
  <div class="composer-mode-panel">
    <div class="edit-source-panel">
      <div class="edit-source-copy">
        ...
        <p class="memory-description">上传原图，或沿用上一轮编辑结果，再输入新的修改要求。</p>
      </div>
      <label class="edit-upload" :class="{ 'has-image': Boolean(previewUrl) }">
        <input type="file" accept="image/*" :disabled="disabled" @change="handleFileChange" />
    ...
    <textarea v-model="form.prompt" class="prompt" placeholder="描述你想如何修改这张图片..." aria-label="图片编辑描述" :disabled="disabled"></textarea>
